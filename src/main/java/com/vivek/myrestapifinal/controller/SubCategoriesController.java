...
public class SubCategoriesController {
	...
	//Get SubCategories by categoies id
	@GetMapping("/getSubCategoriesByCategoriesId/id/{id}")
	public List<SubCategories> getItemBySubCategoriesId(@PathVariable(value="id") String id){
		
		Long subCategoriesId = Long.parseLong(id);		

		List<SubCategories> retSubCategories = new ArrayList<SubCategories>();
		
		List<SubCategories> subCategories = subCategoriesDAO.findAll();
		
		for(int i=0;i<subCategories.size();i++)
		{
			SubCategories it = subCategories.get(i);
			if(it.getCategoriesId() == subCategoriesId)
			{
				retSubCategories.add(it);
			}
		}		
	...
	public ResponseEntity<SubCategories> updateEmployee(@PathVariable(value="id") String id,@Valid @RequestBody SubCategories subCategories){
		...
		cat.setSubCategoriesImage(subCategories.getSubCategoriesImage());
